window.initGRPCForm = function (services, invokeURI, metadataURI, debug, headers) {
	...
	function rebuildRequestForm(newRequest, allowMissing) {
		...
		var comment = $('<th>');
		comment.attr('colspan', '2');
		var str = '';
		if (schema.requestMethodComment) {
			str += schema.requestMethodComment + '\n\n';
		}
		if (schema.messageComments[requestType]) {
			str += schema.messageComments[requestType] + '\n\n';
		}
		if (str) {
			comment.html(converter.makeHtml(str));
		}
	...
	function makeFieldLabelCell(fld, schema) {
		...
		if (fld.comment) {
			...
			container.html(converter.makeHtml(fld.comment));

			var comment = $('<div>');
			comment.append(container);
			comment.addClass('comment');
			comment.addClass('help-tip');
			labelName.append(comment);
		}
	...
	function addEnumToForm(schema, container, parent, value, fld) {
		...
		for (var i = 0; i < enumVals.length; i++) {
			...
			var text = enumVals[i].name;
			if (enumVals[i].comment) {
				text = text + " //" + enumVals[i].comment
			}
			opt.text(text);
	...
	function addOptionalMessage(schema, container, parent, pathLen, value, allowMissing, fld) {
		...
		if (isUnset(value)) {
			...
			// Clear original message form if presented.
			if (container.parent().next().hasClass('optional-message')) {
				container.parent().next().remove();
			}
			return new Input(parent, [], undefined);
		}

		container.parent().after($('<tr class="optional-message"><td colspan="3" class="optmsg-container"></td></tr>'));
		var msgContainer = container.parent().next().children(0);

		return addElementToForm(schema, msgContainer, parent, pathLen, value, allowMissing, fld);
	}
				...
				if (responseData.responses instanceof Array && responseData.responses.length > 0) {
					...
					for (const resp of responseData.responses) {
						...
						if (resp.isError) {
							...
							const area = $('<div>');
							area.jsonViewer(resp.message);
							area.addClass('grpc-response-textarea');
